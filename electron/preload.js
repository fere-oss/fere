...
const rawLogoDevToken = (
  process.env.REACT_APP_LOGO_DEV_TOKEN ||
...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  getContainerLogTail: (containerId, tail) =>
    ipcRenderer.invoke("get-container-log-tail", containerId, tail),

  // Database queries
  getDatabaseTables: (containerId, containerImage) =>
    ipcRenderer.invoke("get-database-tables", containerId, containerImage),
  getTableData: (containerId, containerImage, tableName, limit) =>
    ipcRenderer.invoke("get-table-data", containerId, containerImage, tableName, limit),
  executeDatabaseQuery: (containerId, containerImage, query) =>
    ipcRenderer.invoke("execute-database-query", containerId, containerImage, query),
  createDatabaseTable: (containerId, containerImage, tableName, columns) =>
    ipcRenderer.invoke("create-database-table", containerId, containerImage, tableName, columns),
  connectMongoUri: (uri) => ipcRenderer.invoke("connect-mongo-uri", uri),
  getMongoUriCollectionData: (uri, collectionName, limit) =>
    ipcRenderer.invoke("get-mongo-uri-collection-data", uri, collectionName, limit),
  executeMongoUriQuery: (uri, command) =>
    ipcRenderer.invoke("execute-mongo-uri-query", uri, command),
  connectPostgresUri: (uri) => ipcRenderer.invoke("connect-postgres-uri", uri),
  getPostgresUriTableData: (uri, tableName, limit) =>
    ipcRenderer.invoke("get-postgres-uri-table-data", uri, tableName, limit),
  executePostgresUriQuery: (uri, query) =>
    ipcRenderer.invoke("execute-postgres-uri-query", uri, query),
  connectElasticsearchUri: (baseUrl) => ipcRenderer.invoke("connect-elasticsearch-uri", baseUrl),
  getElasticsearchUriIndexData: (baseUrl, indexName, limit) =>
    ipcRenderer.invoke("get-elasticsearch-uri-index-data", baseUrl, indexName, limit),
  executeElasticsearchUriQuery: (baseUrl, query) =>
    ipcRenderer.invoke("execute-elasticsearch-uri-query", baseUrl, query),
  ...
  startComposeProject: (composeFilePath, services) =>
    ipcRenderer.invoke("start-compose-project", composeFilePath, services),
  ...
  startContainerLogs: (containerId, options) =>
    ipcRenderer.invoke("start-container-logs", containerId, options),
  ...
  onFixProposal: (callback) =>
    ipcRenderer.on("agent:fix-proposal", (_, proposal) => callback(proposal)),
  offFixProposal: () => ipcRenderer.removeAllListeners("agent:fix-proposal"),
  onProactiveFinding: (callback) =>
    ipcRenderer.on("agent:proactive-finding", (_, findings) => callback(findings)),
  offProactiveFinding: () => ipcRenderer.removeAllListeners("agent:proactive-finding"),
  onFindingResolved: (callback) =>
    ipcRenderer.on("agent:finding-resolved", (_, ids) => callback(ids)),
  offFindingResolved: () => ipcRenderer.removeAllListeners("agent:finding-resolved"),
  onFindingWorsened: (callback) =>
    ipcRenderer.on("agent:finding-worsened", (_, findings) => callback(findings)),
