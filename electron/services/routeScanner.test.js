...
test('scanRoutes detects Rails explicit routes, root, and resources', async () => {
  ...
  try {
    ...
    const routes = await scanRoutes(projectDir);
    assert.ok(routes.every(r => r.framework === 'rails'), 'expected all routes tagged as rails');

    // Explicit routes
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/'), 'expected root GET /');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/health'), 'expected GET /health');
    assert.ok(routes.some(r => r.method === 'POST' && r.path === '/api/login'), 'expected POST /api/login');
    assert.ok(routes.some(r => r.method === 'DELETE' && r.path === '/api/logout'), 'expected DELETE /api/logout');

    // resources :users (full CRUD = 5 routes)
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/users'), 'expected GET /users (index)');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/users/:id'), 'expected GET /users/:id (show)');
    assert.ok(routes.some(r => r.method === 'POST' && r.path === '/users'), 'expected POST /users (create)');
    assert.ok(routes.some(r => r.method === 'PATCH' && r.path === '/users/:id'), 'expected PATCH /users/:id (update)');
    assert.ok(routes.some(r => r.method === 'DELETE' && r.path === '/users/:id'), 'expected DELETE /users/:id (destroy)');

    // resources :posts, only: [:index, :show, :create]
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/posts'), 'expected GET /posts (index)');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/posts/:id'), 'expected GET /posts/:id (show)');
    assert.ok(routes.some(r => r.method === 'POST' && r.path === '/posts'), 'expected POST /posts (create)');
    assert.ok(!routes.some(r => r.method === 'PATCH' && r.path === '/posts/:id'), 'expected no PATCH /posts/:id');
    assert.ok(!routes.some(r => r.method === 'DELETE' && r.path === '/posts/:id'), 'expected no DELETE /posts/:id');

    // resource :profile, only: [:show, :update] (singular — no :id)
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/profile'), 'expected GET /profile (show)');
    assert.ok(routes.some(r => r.method === 'PATCH' && r.path === '/profile'), 'expected PATCH /profile (update)');
    assert.ok(!routes.some(r => r.method === 'DELETE' && r.path === '/profile'), 'expected no DELETE /profile');
    ...
    assert.ok(matched.every(r => r.framework === 'rails'), 'expected all matched routes to be rails');
...
test('scanRoutes detects Django path() and re_path() URL patterns', async () => {
  ...
  try {
    ...
    const routes = await scanRoutes(projectDir);
    assert.ok(routes.every(r => r.framework === 'django'), 'expected all routes tagged as django');
    assert.ok(routes.every(r => r.method === 'ALL'), 'expected all django routes to have method ALL');

    assert.ok(routes.some(r => r.path === '/api/users/'), 'expected /api/users/');
    assert.ok(routes.some(r => r.path === '/api/users/<int:pk>/'), 'expected /api/users/<int:pk>/');
    assert.ok(routes.some(r => r.path === '/'), 'expected root path /');
    assert.ok(routes.some(r => r.path === '/articles/(?P<year>[0-9]{4})/'), 'expected re_path with regex');
    ...
    assert.ok(matched.every(r => r.framework === 'django'), 'expected all matched routes to be django');
  } finally {
    fs.rmSync(projectDir, { recursive: true, force: true });
  }
});
...
test('Django and Flask routes are isolated in mixed Python projects', async () => {
  ...
  try {
    ...
    const routes = await scanRoutes(projectDir);
    assert.ok(routes.some(r => r.framework === 'flask' && r.path === '/flask/health'), 'expected flask route');
    assert.ok(routes.some(r => r.framework === 'django' && r.path === '/django/health/'), 'expected django route');
    ...
    assert.ok(flaskRoutes.every(r => r.framework === 'flask'), 'expected flask service to only get flask routes');
    ...
    assert.ok(djangoRoutes.every(r => r.framework === 'django'), 'expected django service to only get django routes');
  } finally {
    fs.rmSync(projectDir, { recursive: true, force: true });
  }
});
...
test('scanRoutes detects Spring Boot annotations including @RequestMapping', async () => {
  ...
  try {
    ...
    const routes = await scanRoutes(projectDir);
    assert.ok(routes.every(r => r.framework === 'spring'), 'expected all routes tagged as spring');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/users'), 'expected GET /users');
    assert.ok(routes.some(r => r.method === 'POST' && r.path === '/users'), 'expected POST /users');
    assert.ok(routes.some(r => r.method === 'PUT' && r.path === '/users/{id}'), 'expected PUT /users/{id}');
    assert.ok(routes.some(r => r.method === 'DELETE' && r.path === '/users/{id}'), 'expected DELETE /users/{id}');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/health'), 'expected GET /health via @RequestMapping');
    assert.ok(routes.some(r => r.method === 'ALL' && r.path === '/status'), 'expected ALL /status via @RequestMapping without method');
...
test('scanRoutes detects Laravel routes and resources', async () => {
  ...
  try {
    ...
    const routes = await scanRoutes(projectDir);
    assert.ok(routes.every(r => r.framework === 'laravel'), 'expected all routes tagged as laravel');

    // Explicit routes
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/health'), 'expected GET /health');
    assert.ok(routes.some(r => r.method === 'POST' && r.path === '/auth/login'), 'expected POST /auth/login');
    assert.ok(routes.some(r => r.method === 'DELETE' && r.path === '/auth/logout'), 'expected DELETE /auth/logout');

    // apiResource (5 routes, no create/edit)
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/posts'), 'expected GET /posts (index)');
    assert.ok(routes.some(r => r.method === 'POST' && r.path === '/posts'), 'expected POST /posts (store)');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/posts/{id}'), 'expected GET /posts/{id} (show)');
    assert.ok(routes.some(r => r.method === 'PUT' && r.path === '/posts/{id}'), 'expected PUT /posts/{id} (update)');
    assert.ok(routes.some(r => r.method === 'DELETE' && r.path === '/posts/{id}'), 'expected DELETE /posts/{id} (destroy)');

    // resource (7 routes, includes create/edit)
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/photos/create'), 'expected GET /photos/create');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/photos/{id}/edit'), 'expected GET /photos/{id}/edit');
...
test('scanRoutes detects Fiber routes and matches via go run', async () => {
  ...
  try {
    ...
    assert.ok(routes.every(r => r.framework === 'fiber'), 'expected all routes tagged as fiber');
    assert.ok(routes.some(r => r.method === 'GET' && r.path === '/api/users'), 'expected GET /api/users');
    assert.ok(routes.some(r => r.method === 'POST' && r.path === '/api/users'), 'expected POST /api/users');
    assert.ok(routes.some(r => r.method === 'DELETE' && r.path === '/api/users/:id'), 'expected DELETE with :id');
