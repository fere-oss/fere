...
async function execMongoUriEval(uri, command, timeout = 30000) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      dockerBin,
      ["run", "--rm", "mongo:7", "mongosh", uri, "--quiet", "--eval", command],
      {
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
...
async function getPostgresTables(containerId) {
  ...
  for (const user of userCandidates) {
    try {
      const args = user ? ["psql", "-U", user, "-t", "-c", sql] : ["psql", "-t", "-c", sql];
...
async function getPostgresTableData(containerId, tableName, limit) {
  ...
  // Get columns first
  const { stdout: colOut } = await execDocker(
    containerId,
    [
      "psql",
      ...userArgs,
  ...
  // Get data as JSON
  const { stdout: dataOut } = await execDocker(
    containerId,
    [
      "psql",
      ...userArgs,
...
async function getMySQLTables(containerId) {
  ...
  try {
    const { stdout } = await execDocker(
      containerId,
      ["mysql", ...authArgs, "-e", "SHOW TABLES;", "--skip-column-names"],
      10000,
    );
    const tables = stdout
      .split("\n")
    ...
    try {
      const { stdout: dbOut } = await execDocker(
        containerId,
        ["mysql", ...authArgs, "-e", "SHOW DATABASES;", "--skip-column-names"],
        10000,
      );
      const databases = dbOut
        .split("\n")
        .map((l) => l.trim())
        .filter((l) => l && !MYSQL_SYSTEM_DBS.has(l));

      if (databases.length > 0) {
        const { stdout } = await execDocker(
          containerId,
          ["mysql", ...authArgs, "-D", databases[0], "-e", "SHOW TABLES;", "--skip-column-names"],
          10000,
        );
        const tables = stdout
          .split("\n")
...
async function getMongoCollectionData(containerId, collectionName, limit) {
  ...
  const commandVariants = [
    [
      ...
      ...authArgs,
      "--eval",
      `JSON.stringify(db.getCollection("${safeName}").find().limit(${limit}).toArray())`,
    ],
    [
      "mongo",
      "--quiet",
...
async function executePostgresQuery(containerId, query) {
  try {
    const envUser = await resolvePostgresUser(containerId);
    const userArgs = envUser ? ["-U", envUser] : ["-U", "postgres"];
    const { stdout, stderr } = await execDockerWithInput(
      containerId,
      ["psql", ...userArgs],
      query,
      30000,
    ...
    if (query.trim().toLowerCase().startsWith("select")) {
      ...
      try {
        const { stdout: jsonOut } = await execDockerWithInput(
          containerId,
          ["psql", ...userArgs, "-t"],
          jsonQuery,
          30000,
...
`;
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        collections = parsed.map((name) => String(name).trim()).filter(Boolean);
      }
    } catch {
    ...
    return {
      error: error.message || "Could not connect to MongoDB URI",
      tables: [],
      dbType: "mongodb",
    };
  }
...
async function getMongoUriCollectionData(uri, collectionName, limit = 100) {
  if (!uri || !collectionName) {
    return {
      error: "MongoDB URI and collection are required",
      columns: [],
      rows: [],
      dbType: "mongodb",
    };
  ...
  try {
    ...
    return {
      error: error.message || "Could not query MongoDB URI collection",
      columns: [],
      rows: [],
      dbType: "mongodb",
    };
...
async function connectPostgresUri(uri) {
  ...
  try {
    ...
    return {
      error: error.message || "Could not connect to PostgreSQL URI",
      tables: [],
      dbType: "postgresql",
    };
  }
...
async function getPostgresUriTableData(uri, tableName, limit = 100) {
  if (!uri || !tableName) {
    return {
      error: "PostgreSQL URI and table name are required",
      columns: [],
      rows: [],
      dbType: "postgresql",
    };
  ...
  try {
    ...
    return {
      error: error.message || "Could not query PostgreSQL URI table",
      columns: [],
      rows: [],
      dbType: "postgresql",
    };
...
async function executePostgresUriQuery(uri, query) {
  ...
  try {
    ...
    return {
      output:
        typeof result.rowCount === "number"
          ? `${result.command || "QUERY"} ${result.rowCount}`
          : result.command || "Query executed",
    ...
    return {
      error: error.message || "Failed to execute PostgreSQL URI query",
      dbType: "postgresql",
    };
  }
}
    ...
    const req = transport.request(reqOptions, (res) => {
      ...
      res.on("end", () => {
        ...
        if (res.statusCode >= 400) {
          ...
          try {
            message = JSON.parse(body)?.error?.reason || message;
          } catch {}
...
async function connectElasticsearchUri(baseUrl) {
  try {
    ...
    const indices = await esFetchWithFallback(
      `${url}/_cat/indices?format=json&h=index,health,status,docs.count,store.size`,
    );
...
async function getElasticsearchUriIndexData(baseUrl, indexName, limit = 100) {
  try {
    ...
    return {
      columns: [],
      rows: [],
      error: error.message || "Failed to fetch index data",
...
async function executeElasticsearchUriQuery(baseUrl, query) {
  try {
    ...
    try {
      ...
      return {
        error: 'Query must be valid JSON. Example: {"query": {"match_all": {}}}',
        dbType: "elasticsearch",
      };
    }
    ...
    return {
      error: error.message || "Failed to execute Elasticsearch query",
      dbType: "elasticsearch",
    };
  }
}
...
async function createMySQLTable(containerId, tableName, columns) {
  ...
  const primaryKeyDef = primaryKeyCol
    ? `, PRIMARY KEY (${escapeMySQLIdent(primaryKeyCol.name)})`
    : "";
...
async function createMongoCollection(containerId, collectionName, columns) {
  ...
  const safeName = escapeMongoJsString(collectionName);
  const commands = [`db.createCollection("${safeName}")`];
