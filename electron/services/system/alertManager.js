...
function recordAlertEventHistoryOnly(type, node) {
  alertHistoryWriteQueue = alertHistoryWriteQueue
    .then(() => {
...
function clearAlertHistory() {
  return (alertHistoryWriteQueue = alertHistoryWriteQueue
    .then(() => {
      writeAlertHistory([]);
      return { success: true };
    })
...
function evaluateAlerts(nodes) {
  ...
  for (const node of nodes) {
    ...
    if (
      ...
      if (
        ...
        prev.lastNotifiedAt = now;
      } else if (previousContainerState !== "running" && currentContainerState === "running") {
  ...
  for (const [nodeId, state] of nodeStates) {
    if (!currentNodeIds.has(nodeId)) {
      recordAlertEvent(
        "service-gone",
        { id: nodeId, name: state.name, type: "service" },
        "",
        false,
      );
