...
const ROUTE_PATTERNS = {
  ...
  // Hono: app.get("/path")
  hono: [/(?:app|hono)\.(get|post|put|delete|patch|all)\s*\(\s*["'`]([^"'`]+)["'`]/gi],
  // Koa: router.get("/path")
  koa: [/router\.(get|post|put|delete|patch|all)\s*\(\s*["'`]([^"'`]+)["'`]/gi],
  // Plain Node.js http server with object-based routes: "GET /path": handler
  // Matches patterns like: "GET /", "POST /users", 'DELETE /items/:id'
  "node-http": [/["'](GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|ALL)\s+(\/[^"']*)["']\s*:/gi],
  // Gin: r.GET("/path", handler), router.POST("/path", handler)
  gin: [/\w+\.(GET|POST|PUT|DELETE|PATCH)\s*\(\s*["`]([^"`]+)["`]/g],
  // Echo: e.GET("/path", handler), echo.POST("/path", handler)
  echo: [/\w+\.(GET|POST|PUT|DELETE|PATCH)\s*\(\s*["`]([^"`]+)["`]/g],
  ...
  rails: [/(get|post|put|patch|delete)\s+['"]([^'"]+)['"]/gi],
  ...
  spring: [/@(Get|Post|Put|Delete|Patch)Mapping\s*\(\s*(?:value\s*=\s*)?["']([^"']+)["']/gi],
  // Laravel: Route::get('/path', ...), Route::post('/path', ...), etc.
  laravel: [/Route::(get|post|put|patch|delete)\s*\(\s*['"]([^'"]+)['"]/gi],
  // Fiber: app.Get("/path", handler), app.Post("/path", handler)
  fiber: [/\w+\.(Get|Post|Put|Delete|Patch)\s*\(\s*["`]([^"`]+)["`]/g],
};
...
function detectFramework(filePath, content) {
  ...
  if (ext === ".py") {
    if (
      ...
      (content.includes("from starlette") || content.includes("from fastapi"))
    ) {
      return "fastapi";
    }
  }
...
function extractRoutes(filePath, content, framework) {
  ...
  if (framework === "nextjs") {
    ...
    if (routePath) {
      ...
      if (
        /socket\s*\.\s*on\s*\(\s*['"]connection['"]|new\s+WebSocketServer|wss\.on\s*\(\s*['"]connection['"]/.test(
          content,
        )
      ) {
        methods.push("WS");
  ...
  if (framework === "laravel") {
    ...
    while ((resMatch = resourcePattern.exec(content)) !== null) {
      const name =
        resMatch[0].includes("apiResource") || resMatch[0].includes("apiR")
          ? resMatch[1]
          : resMatch[1];
  ...
  if (framework === "rails") {
    ...
    while ((resMatch = resourcesPattern.exec(content)) !== null) {
      ...
      for (const action of actions) {
        if (action === "index")
          routes.push({ method: "GET", path: `/${name}`, file: filePath, framework });
        if (action === "show")
          routes.push({ method: "GET", path: `/${name}/:id`, file: filePath, framework });
        if (action === "create")
          routes.push({ method: "POST", path: `/${name}`, file: filePath, framework });
        if (action === "update")
          routes.push({ method: "PATCH", path: `/${name}/:id`, file: filePath, framework });
        if (action === "destroy")
          routes.push({ method: "DELETE", path: `/${name}/:id`, file: filePath, framework });
      }
    }
    ...
    while ((singMatch = resourcePattern.exec(content)) !== null) {
      ...
      for (const action of actions) {
        ...
        if (action === "create")
          routes.push({ method: "POST", path: `/${name}`, file: filePath, framework });
        if (action === "update")
          routes.push({ method: "PATCH", path: `/${name}`, file: filePath, framework });
        if (action === "destroy")
          routes.push({ method: "DELETE", path: `/${name}`, file: filePath, framework });
      }
    }
  }
...
function matchRoutesToService(routes, service) {
  ...
  // Rails
  if (
    command.includes("rails") ||
    command.includes("puma") ||
    command.includes("unicorn") ||
    command.includes("passenger")
  )
    serviceFrameworks.add("rails");
  // Spring Boot
  if (
    command.includes("spring") ||
    command.includes("java") ||
    command.includes("mvn") ||
    command.includes("gradle")
  )
    serviceFrameworks.add("spring");
  // Laravel
  if (command.includes("artisan") || command.includes("laravel") || command.includes("php"))
    serviceFrameworks.add("laravel");
  ...
  for (const route of routes) {
    ...
    if (serviceFrameworks.size > 0 && route.framework && !serviceFrameworks.has(route.framework))
      continue;
