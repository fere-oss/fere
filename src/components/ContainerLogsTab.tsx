...
const LogRow = memo(
  ({ index, style, logs, showTimestamps, highlightMatches }: RowComponentProps<LogRowProps>) => {
    ...
    return (
      ...
      >
        {showTimestamps && <span className="unified-log-time">{log.formattedTime}</span>}
        <span className="unified-log-container" style={{ color: log.containerColor }}>
          {log.containerName}
        </span>
        ...
        <span className="unified-log-text" title={log.line}>
          {typeof highlighted === "string"
            ...
            : highlighted.map((part, i) =>
                part.match ? (
                  <mark key={i} className="unified-log-highlight">
                    {part.text}
                  </mark>
                ) : (
...
export function ContainerLogsTab({ containers, initialSelectedId }: ContainerLogsTabProps) {
  ...
  const [filterLevel, setFilterLevel] = useState<"all" | "error" | "warn" | "info" | "debug">(
    "all",
  );
  ...
  useEffect(() => {
    const availableIds = new Set(
      containers.map((c) => c.containerId).filter((id): id is string => Boolean(id)),
    );
  ...
  const handleCopy = useCallback(async () => {
    const text = filteredLogs
      .map((log) => `[${log.formattedTime}] [${log.containerName}] ${log.line}`)
      .join("\n");
          ...
          <div className="unified-logs-containers">
            <div className="unified-logs-containers-header">
              ...
              <div className="unified-logs-containers-actions">
                <button onClick={selectAll} className="unified-logs-action-btn">
                  All
                </button>
                <button onClick={deselectAll} className="unified-logs-action-btn">
                  None
                </button>
              </div>
            </div>
