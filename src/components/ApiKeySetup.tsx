...
export function ApiKeySetup({ onKeyChanged }: Props) {
  ...
  const keyStorageDetails = (
    <ul className="agp-key-details agp-key-details-inline">
      ...
      <li>
        Used only for direct API calls to <code>api.openai.com</code>
      </li>
    </ul>
  );
      ...
      {hasKey ? (
        <>
          ...
          <div className="agp-key-actions">
            <button className="agp-key-details-toggle" onClick={() => setShowDetails(!showDetails)}>
          ...
          {showDetails && (
            <ul className="agp-key-details">
              <li>
                Encrypted at rest using macOS Keychain — same mechanism used by 1Password and VS
                Code
              </li>
              ...
              <li>
                Alternatively, set <code>OPENAI_API_KEY</code> in your shell profile to skip in-app
                storage entirely
              </li>
            </ul>
          )}
        ...
        <>
          <div className="agp-key-prompt">
            <span className="agp-key-prompt-label">
              Add your OpenAI API key for unlimited calls
            </span>
          </div>
          {keyStorageDetails}
