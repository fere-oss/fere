...
type ResultCategory = "Services" | "Ports" | "Routes" | "Projects" | "Actions" | "Service Actions";
...
function focusNode(nodeId: string, select = false) {
  window.dispatchEvent(new CustomEvent("fere:debug-focus-node", { detail: { nodeId, select } }));
}
...
export const CommandPalette = forwardRef<CommandPaletteHandle, CommandPaletteProps>(
  function CommandPalette(
    ...
    const findTabForNode = useCallback(
    ...
    const focusServiceNode = useCallback(
      (node: GraphNode) => {
    ...
    const results = useMemo(() => {
      ...
      for (const node of graph.nodes) {
        ...
        serviceResults.push({
          ...
          healthColor: (HEALTH_COLORS[node.healthStatus] || HEALTH_COLORS.yellow).color,
      ...
      for (const node of graph.nodes) {
        ...
        if (node.isDockerContainer && node.containerId) {
          ...
          // View logs
          if (score(`View ${node.name} logs`) !== -1 || score(node.name) !== -1) {
            if (canAdd("Service Actions")) {
              items.push({
                id: `logs-${node.id}`,
                category: "Service Actions",
        ...
        // Investigate
        if (score(`Investigate ${node.name}`) !== -1 || score(node.name) !== -1) {
          if (canAdd("Service Actions")) {
    ...
    useEffect(() => {
      ...
      const selected = dropdownRef.current.querySelector(".command-palette-item-selected");
      if (selected) {
