...
export function useDatabasePage(node: GraphNode): UseDatabasePageResult {
  ...
  const [remoteUriDbType, setRemoteUriDbType] = useState<
    "mongodb" | "postgresql" | "elasticsearch" | null
  >(null);
  ...
  const detectUriDbType = useCallback(
    (uri: string): "mongodb" | "postgresql" | "elasticsearch" | null => {
  ...
  const saveRecentMongoUri = useCallback(
    (uri: string) => {
  ...
  const applyRemoteTableDefaults = useCallback(
    (remoteTables: string[]) => {
  ...
  useEffect(() => {
    ...
    const loadTables = async () => {
      ...
      const isElasticsearch =
        ...
        nodeName.includes("opensearch");
      if (isElasticsearch) {
        const esPort =
          node.ports.find((p) => p.port === 9200 || p.port === 9201)?.port ||
          node.ports[0]?.port ||
          9200;
      ...
      try {
        ...
        const result: DatabaseTablesResult = await window.electronAPI.getDatabaseTables(
          containerId,
          containerImage,
        );
        if (isCancelled) return;
  ...
  const loadTableData = useCallback(
    async (tableName: string) => {
      ...
      try {
        ...
        const result =
          remoteMongoMode && remoteMongoUri
            ? remoteUriDbType === "mongodb" && window.electronAPI.getMongoUriCollectionData
              ...
              : remoteUriDbType === "postgresql" && window.electronAPI.getPostgresUriTableData
                ...
                : remoteUriDbType === "elasticsearch" &&
                    window.electronAPI.getElasticsearchUriIndexData
                  ? await window.electronAPI.getElasticsearchUriIndexData(
                      remoteMongoUri,
                      tableName,
                      100,
                    )
    ...
    try {
      if (remoteMongoMode && remoteMongoUri) {
        ...
        if (remoteUriDbType === "mongodb" && window.electronAPI?.connectMongoUri) {
          ...
          remoteUriDbType === "elasticsearch" &&
          window.electronAPI?.connectElasticsearchUri
        ) {
  ...
  const testMongoUriConnection = useCallback(async () => {
    ...
    try {
      ...
      const itemLabel =
        uriType === "mongodb" ? "collections" : uriType === "elasticsearch" ? "indices" : "tables";
      setMongoUriStatus("ok");
  ...
  const normalizeExecutableQuery = useCallback(
    (rawQuery: string) => {
      ...
      if (dbType === "mongodb") {
        ...
        const hasPrintWrapper =
          /\bprint\s*\(/i.test(withoutTrailingSemicolon) ||
          /\bconsole\.log\s*\(/i.test(withoutTrailingSemicolon);
        ...
        const isWriteOp =
          /\.(insertOne|insertMany|updateOne|updateMany|replaceOne|deleteOne|deleteMany|drop|createCollection)\s*\(/i.test(
        ...
        remoteMongoMode && remoteMongoUri
          ? remoteUriDbType === "mongodb" && window.electronAPI.executeMongoUriQuery
            ...
            : remoteUriDbType === "postgresql" && window.electronAPI.executePostgresUriQuery
              ...
              : remoteUriDbType === "elasticsearch" &&
                  window.electronAPI.executeElasticsearchUriQuery
                ? await window.electronAPI.executeElasticsearchUriQuery(
                    remoteMongoUri,
                    executableQuery,
                  )
                : { error: "Remote database mode is not available for this URI type" }
          : await window.electronAPI.executeDatabaseQuery(
              containerId,
              containerImage,
              executableQuery,
            );
  ...
  const handleCreateTable = useCallback(
    async (tableName: string, columns: ColumnDefinition[]) => {
      if (remoteMongoMode && remoteMongoUri && dbType === "mongodb") {
        ...
        const result = await window.electronAPI.executeMongoUriQuery(
          remoteMongoUri,
      ...
      const result = await window.electronAPI.createDatabaseTable(
        containerId,
        containerImage,
        tableName,
        columns,
  ...
  const handleDeleteTable = useCallback(async () => {
    ...
    try {
      ...
      setActionError(
        `Failed to delete ${dbType === "mongodb" ? "collection" : "table"}: ${deleteError instanceof Error ? deleteError.message : "Unknown error"}`,
      );
    } finally {
  ...
  const handleDeleteRow = useCallback(
    async (rowIndex: number, row: Record<string, unknown>) => {
      ...
      try {
        ...
        const result =
          remoteMongoMode &&
          remoteMongoUri &&
          remoteUriDbType === "postgresql" &&
          window.electronAPI.executePostgresUriQuery
        ...
        setActionError(
          `Failed to delete row: ${deleteError instanceof Error ? deleteError.message : "Unknown error"}`,
        );
      } finally {
        setDeletingRow(null);
      }
  ...
  const setSelectedRemoteDbWithSelection = useCallback(
    (dbName: string) => {
  ...
  const setSelectedRemoteCollectionWithLoad = useCallback(
